...
# ifaces=$(ls /sys/class/net | grep -E '^(eth|wlan|enp|wlp|br)')
...
update_rate() {
  ...
  for iface in $ifaces; do
    read tmp_rx < "/sys/devices/virtual/net/${iface}/statistics/rx_bytes"
    read tmp_tx < "/sys/devices/virtual/net/${iface}/statistics/tx_bytes"
    rx=$(( rx + tmp_rx ))
    tx=$(( tx + tmp_tx ))
  done
